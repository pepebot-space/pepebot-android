...
public class TermuxAppSharedPreferences extends AppSharedPreferences {
     ...
     * @return Returns the {@link TermuxAppSharedPreferences}. This will
     *         {@code null} if an exception is raised.
     */
    @Nullable
    public static TermuxAppSharedPreferences build(@NonNull final Context context) {
        // Use runtime package name instead of TERMUX_PACKAGE_NAME since the host app
        // may have a different applicationId (e.g. com.terminal.pepebot)
        Context termuxPackageContext = PackageUtils.getContextForPackage(context, context.getPackageName());
     ...
     * Get {@link TermuxAppSharedPreferences}.
     *
     * @param context        The {@link Context} to use to get the {@link Context}
     *                       of the
     *                       {@link TermuxConstants#TERMUX_PACKAGE_NAME}.
     * @param exitAppOnError If {@code true} and failed to get package context, then
     *                       a dialog will
    ...
    public static TermuxAppSharedPreferences build(@NonNull final Context context, final boolean exitAppOnError) {
        // Use runtime package name instead of TERMUX_PACKAGE_NAME since the host app
        // may have a different applicationId (e.g. com.terminal.pepebot)
        Context termuxPackageContext = TermuxUtils.getContextForPackageOrExitApp(context, context.getPackageName(),
                exitAppOnError);
        if (termuxPackageContext == null)
            return null;
        else
    ...
    public static int[] getDefaultFontSizes(Context context) {
        ...
        // This is a bit arbitrary and sub-optimal. We want to give a sensible default
        // for minimum font size
